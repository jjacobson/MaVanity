 ...
 */
public class FeetEffect extends BukkitRunnable implements ParticleEffect {

    private MaVanity plugin;
    private Player player;
    private Random random;

    public FeetEffect(MaVanity plugin, Player player) {
        this.plugin = plugin;
        this.player = player;
    ...
    public void run() {
        ...
        for (int i = 0; i < 10; i++) {
            Location location = player.getLocation().add(getRandomChange(), getRandomChange() + 0.2, getRandomChange());
    ...
    private double getRandomChange() {
        return (double) (random.nextInt(11) - 5) / 10.0;
    }
}
